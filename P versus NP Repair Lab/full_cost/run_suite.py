...
def main():
    ...
    ap.add_argument('--cases', default='2:0,2:1,2:2,2:3,2:6,3:0,3:2,3:4,3:6,4:0,4:2,4:4,4:6')
    args = ap.parse_args()
    root = pathlib.Path(__file__).resolve().parent
    out = pathlib.Path(args.output).resolve()
    out.mkdir(parents=True, exist_ok=True)
    binary = out / 'constructor'
    compile_argv = ['g++', '-std=c++20', '-O2', '-Wall', '-Wextra', '-Wpedantic', str(root / 'constructor.cpp'), '-o', str(binary)]
    ...
    report['all_declared_cases_completed']=True
    target.write_text(json.dumps(report, indent=2)+'\n')
